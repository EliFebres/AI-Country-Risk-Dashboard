...
export default function Masthead({
  ...
  const dataAsOf = useMemo(() => {
    ...
    if (typeof dataTimestamp === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(dataTimestamp)) {
      // Parse a bare ISO date as a *local* date — `new Date('YYYY-MM-DD')` would
      // read it as UTC midnight and render the prior day in negative-UTC locales.
      const [y, m, d] = dataTimestamp.split('-').map(Number);
      dt = new Date(y, m - 1, d);
    } else {
      dt = dataTimestamp instanceof Date ? dataTimestamp : new Date(dataTimestamp);
    }
  ...
  return (
    <header className="topbar" aria-label="Masthead">
      <div className="tb-left">
        ...
        <a
          className="gh-link"
          href="https://github.com/EliFebres/AI-Country-Risk-Dashboard"
          target="_blank"
